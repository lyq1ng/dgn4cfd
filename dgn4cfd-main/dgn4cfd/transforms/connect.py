...
def connect_knn(pos: torch.Tensor,
                k: int,
                period: Tuple[float] = None) -> Tuple[torch.Tensor, torch.Tensor]:
...
class MeshEllipse:
    """Transformation class to create a mesh around an ellipse."""

    def __call__(
        self,
        graph: Graph,
    ) -> Graph:
        # Compute the edge_index by sorting the nodes by angle
        ...
        graph.edge_index = torch.cat([
            torch.stack((idx, torch.roll(idx, -1, 0)), dim=0),
            torch.stack((idx, torch.roll(idx,  1, 0)), dim=0),
        ], dim=1)
        # Sort each edge by increasong value of the second node
        idx = torch.argsort(graph.edge_index[1])
        graph.edge_index = graph.edge_index[:, idx]
        # Compute the edge_attr
